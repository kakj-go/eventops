...
类似于 `docker` 命令, 你可以登录多个账号进行账号的切换操作
...
#### pipelineDefinition
流水线定义，类似 docker image

#### pipeline
流水线定义的 runtime, 类似 docker container

#### actuatorDefinition
声明执行器，目前有 3 类 [os, docker, k8s]。其中有个 tag 字段可以为执行器声明别名

创建 docker actuator

`eoctl actuator apply -f example/actuator/dockerActuator.yaml`

注意:
1. docker actuator 需要 docker daemon 开启 tcp

创建 docker ssh actuator

`eoctl actuator apply -f example/actuator/dockerActuatorSsh.yaml`

注意:
1. docker ssh actuator 需要 docker daemon 开启 tcp
2. 使用 ssh 通道 docker ip 地址可以使用 127.0.0.1

创建 docker tunnel actuator

`eoctl actuator apply -f example/actuator/dockerActuatorSsh.yaml`

注意:
1. docker tunnel actuator 需要 docker daemon 开启 tcp
2. 使用 tunnel 后 docker ip 地址可以使用 127.0.0.1
3. docker tunnel actuator 需要在宿主机上运行 `./client --id=yaml中声明的clientId --token=yaml中声明的clientToken --user=你的用户名`

创建 k8s actuator

`eoctl actuator apply -f example/actuator/k8sActuator.yaml`

注意:
1. k8s actuator 的 config 配置中的 server:http://xxxx:6443 地址需要要能被 eventops 访问。 一般直接拷贝过来的都是域名地址，可能无法解析

创建 k8s tunnel actuator

`eoctl actuator apply -f example/actuator/k8sActuatorTunnel.yaml`

注意:
1. k8s tunnel actuator 的 config 配置中的 server:http://xxxx:6443 地址需要能被 client 宿主机访问

创建 os actuator

`eoctl actuator apply -f example/actuator/osActuator.yaml`

创建 os tunnel actuator

`eoctl actuator apply -f example/actuator/osActuatorTunnel.yaml`

注意:
1. os tunnel actuator 中的 ip 地址和端口要能被 client 宿主机访问


#### task
流水线定义中的任务，分 4 种类型 [os, docker, k8s, pipeline] 其中 pipeline 类型效果是执行另一个流水线
...
docker 和 k8s 类型的 task image 是容器镜像
...
#### event
事件，用户使用 eoctl 发送或者直接调用 server 的 api

#### triggerDefinition
触发器定义，用于多个流水线和事件进行绑定，可以对事件内容进行过滤，运行流水线的时候还可以将事件的内容传递给流水线